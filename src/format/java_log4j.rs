...
impl JavaLog4jFormatter {
    ...
    pub fn format(&self, ctx: &EventContext) -> String {
        ...
        let class = ctx
            ...
            .get("java_class")
            .map(|s| s.as_str())
            .unwrap_or("com.app.Main");
        let message = ctx
            .fields
            .get("message")
            .map(|s| s.as_str())
            .unwrap_or("event generated");
...
mod tests {
    ...
    fn formats_valid_log4j() {
        ...
        ctx.fields
            .insert("thread_name".into(), "http-nio-8080-exec-3".into());
        ctx.fields
            .insert("java_class".into(), "com.app.service.UserService".into());
        ctx.fields
            .insert("message".into(), "NullPointerException".into());
        let output = formatter.format(&ctx);

        assert!(
            output.contains("[http-nio-8080-exec-3]"),
            "missing thread: {output}"
        );
        assert!(output.contains("ERROR"), "missing level: {output}");
        assert!(
            output.contains("com.app.service.UserService"),
            "missing class: {output}"
        );
        assert!(
    ...
    fn formats_valid_logback() {
        ...
        assert!(
            output.chars().next().unwrap().is_ascii_digit(),
            "should start with time: {output}"
        );
        assert!(
            !output.starts_with("20"),
            "should not start with year: {output}"
        );
