...
impl ClfFormatter {
    ...
    pub fn format(&self, ctx: &EventContext) -> String {
        let ip = ctx.fields.get("src_ip").map(|s| s.as_str()).unwrap_or("127.0.0.1");
        let user = ctx.fields.get("username").map(|s| s.as_str()).unwrap_or("-");
        let method = ctx.fields.get("http_method").map(|s| s.as_str()).unwrap_or("GET");
        let path = ctx.fields.get("http_path").map(|s| s.as_str()).unwrap_or("/");
        let status = ctx.fields.get("http_status").map(|s| s.as_str()).unwrap_or("200");
        let bytes = ctx.fields.get("http_bytes").map(|s| s.as_str()).unwrap_or("0");
        ...
        let user_agent = ctx.fields.get("user_agent").map(|s| s.as_str()).unwrap_or("-");
...
mod tests {
    ...
    fn formats_valid_clf() {
        ...
        assert!(output.starts_with("192.168.1.55 - jsmith ["), "bad CLF start: {output}");
