...
fn register_fake_data(lua: &Lua) -> Result<()> {
    ...
    globals.set("fake_ipv4", lua.create_function(|_, ()| {
        let ip: Ipv4Addr = IPv4().fake();
        Ok(ip.to_string())
    })?)?;

    globals.set("fake_ipv6", lua.create_function(|_, ()| {
        let ip: String = IPv6().fake();
        Ok(ip)
    })?)?;

    globals.set("fake_hostname", lua.create_function(|_, ()| {
    ...
    globals.set("fake_username", lua.create_function(|_, ()| {
        let name: String = Username().fake();
        Ok(name)
    })?)?;

    globals.set("fake_user_agent", lua.create_function(|_, ()| {
        let ua: String = UserAgent().fake();
        Ok(ua)
    })?)?;

    globals.set("fake_http_method", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
    ...
    globals.set("fake_http_path", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
        let paths = [
    ...
    globals.set("fake_http_status", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
        let statuses: [i64; 17] = [
            200, 200, 200, 200, 201, 204, 301, 302, 304,
    ...
    globals.set("fake_log_level", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
    ...
    globals.set("fake_java_class", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
        let classes = [
            "com.app.service.UserService", "com.app.service.OrderService",
            "com.app.controller.AuthController", "com.app.repository.UserRepository",
            "com.app.security.JwtFilter", "com.app.config.DataSourceConfig",
            "com.app.handler.ExceptionHandler", "com.app.service.NotificationService",
            "com.app.gateway.PaymentGateway", "com.app.scheduler.CleanupTask",
        ];
        let idx = rng.random_range(0..classes.len());
        Ok(classes[idx].to_string())
    })?)?;

    globals.set("fake_thread_name", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
        let names = [
    ...
    globals.set("fake_message", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
        let messages = [
            "Connection established successfully", "Request processed in 23ms",
            "User authentication completed", "Cache miss for key user:session",
            "Health check passed", "Scheduled task executed",
            "Configuration reloaded", "New connection from client",
            "Database query completed in 45ms", "Service started successfully",
            "Incoming request received", "Response sent to client",
            "Worker thread spawned", "Memory usage within threshold",
            "Periodic cleanup completed", "Session expired for user",
            "Rate limit exceeded for client", "SSL handshake completed",
            "DNS resolution completed", "Retry attempt 2 of 3",
            "Transaction committed successfully", "File upload completed",
            "Notification sent to queue", "Circuit breaker state: CLOSED",
    ...
    globals.set("fake_port", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
        Ok(rng.random_range(1024i64..65535))
    })?)?;

    globals.set("fake_protocol", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
    ...
    globals.set("fake_action", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
    ...
    globals.set("fake_severity", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
...
fn register_timestamps(lua: &Lua) -> Result<()> {
    ...
    globals.set("now_iso", lua.create_function(|_, ()| {
        Ok(chrono::Utc::now().format("%Y-%m-%dT%H:%M:%S%.3fZ").to_string())
    })?)?;

    globals.set("now_epoch", lua.create_function(|_, ()| {
        Ok(chrono::Utc::now().timestamp())
    })?)?;

    globals.set("now_rfc3339", lua.create_function(|_, ()| {
...
fn register_utilities(lua: &Lua) -> Result<()> {
    ...
    globals.set("uuid", lua.create_function(|_, ()| {
        let mut rng = rand::rng();
    ...
    globals.set("fake_int", lua.create_function(|_, (min, max): (i64, i64)| {
        let mut rng = rand::rng();
        Ok(rng.random_range(min..=max))
    })?)?;

    globals.set("int_range", lua.create_function(|_, (min, max): (i64, i64)| {
        let mut rng = rand::rng();
        Ok(rng.random_range(min..=max))
    })?)?;

    // pick(table) — pick a random element from a Lua table (1-based array)
    globals.set("pick", lua.create_function(|_, table: Table| {
    ...
    globals.set("weighted_bool", lua.create_function(|_, p: f64| {
        let mut rng = rand::rng();
